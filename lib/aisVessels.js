...
function onmessage(delta) {
	...
	//dont do self
	//console.log(delta);
	if(vessel === 'vessels.motu') return;
	var now = new Date().getTime();
	//10 min max
	now=now-600000;
	if(delta.updates){
		delta.updates.forEach(function(ais){
			var found, coord, radians, sog, mmsi, vhf, cog;
			ais.values.forEach(function(value) {
				//console.log(value);
				if(value.path === 'navigation.position'){
					//console.log(value.value.latitude);
					coord = ol.proj.transform([value.value.longitude, value.value.latitude], 'EPSG:4326', 'EPSG:3857');
				}
				if(value.path === 'navigation.courseOverGroundTrue'){
					cog=value.value;
					radians = util.toRad(value.value-90);
				}
