...
//vessel circle
		var circleLine = new ol.geom.Circle([], 1000, 'XY');
		var vesselCir = new ol.Feature({
			geometry: circleLine,
			name: 'Vessel circle'
		});
		vesselCir.setStyle(
...
function setVesselOverlay(map, vessPos, vessTrack, vessHdg, vessAppWind, vessTrueWind, vessCir) {
	...
	featureOverlay.getSource().addFeature(vessCir);
...
//							});
		...
		if (coord && !isNaN(coord[0]) && !isNaN(coord[1])) {
			...
			// do the circleLine
			circleLine.setCenterAndRadius(coord, 1000, 'XY');
