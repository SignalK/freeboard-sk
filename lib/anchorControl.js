                    ...
                    "value" : maxRadius
							  ...
							  },
							  {
								  ...
								  value: {
									  ...
									  "zones": [
										  {lower: "0", upper: maxRadius, state : "normal"},
										  {lower: maxRadius, upper: 999999, state: "alarm"}
									  ]
								  }
							  }
...
function onmessage(delta) {
	...
	if(delta.updates){
      ...
      //console.log(value);
        ...
        //console.log('Anchor Pos: (' + currentLat + ',' + currentLon + ')');
      }

      if (value.path === 'navigation.position.latitude') {
       		//console.log('Anchor Pos:'+value);
       	  currentLat = value;
      }
...
$("#anchorPopupReset").on('click', function() {
	...
	lat=currentLat;
	lon=currentLon;
	$("#anchorPopupLat").text(lat.value);
	$("#anchorPopupLon").text(lon.value);
	var coord = ol.proj.transform([lon,lat], 'EPSG:4326', 'EPSG:3857');
	anchorCircle.setCenterAndRadius(coord,maxRadius);
