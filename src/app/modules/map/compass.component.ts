import {Component, ElementRef, Input, ViewChild, ChangeDetectionStrategy,
		Renderer2} from '@angular/core';

//** Base class **
class SvgDialBase {
	//** config variables **
	protected minValue: number= 0;		//** min scale value
	protected maxValue: number= 360;	//** max scale value
	protected minAngle: number= 0;		//** pointer angle when showing min value 
	protected maxAngle: number= 360;	//** pointer angle when showing max value 
	private _scale: number= 1;			//** scaling variables
	protected pointers = {				//** dial pointers
		value: null,
		lo: null,
		hi: null
	}
	...
	private _init() {
		let a: number=  (this.minAngle > this.maxAngle) ? (360-this.minAngle) + this.maxAngle : this.maxAngle-this.minAngle;
		let b: number= this.maxValue - this.minValue;
...
export class CompassComponent extends SvgDialBase  {
    ...
    private ptrOffset: number= 0;      // ** pointer offset value
	...
	ngOnChanges(changes) { this.updateDial(changes) }

    updateDial(changes=null) {
        let d={ heading: null, needle: null, windtrue: null, windapparent: null, speed: null };
        d.heading= (changes && changes.heading) ? changes.heading.currentValue : this.heading;
        d.needle= (changes && changes.needle) ? changes.needle.currentValue : this.needle;
        d.windtrue= (changes && changes.hi) ? changes.windtrue.currentValue : this.windtrue;
        d.windapparent= (changes && changes.windapparent) ? changes.windapparent.currentValue : this.windapparent;
        d.speed= (changes && changes.speed) ? changes.speed.currentValue : this.speed;   
		...
		if(this.pointers.value) {
			...
			if (d.heading!=null) {
				...
				this.renderer.setAttribute (
					...
					'transform', 'rotate(' + val + ' 100 100)'
				);
				this.renderer.setProperty (
					this.headingtext.nativeElement,    
					'innerHTML', this.valueStr
				);
            }
            else {
				this.renderer.setAttribute (
					this.dial.nativeElement,    
					'transform', 'rotate(' +  (0-this.getAngle(0)) + ' 100 100)'
				);                
                this.renderer.setProperty (
                    this.headingtext.nativeElement,    
                    'innerHTML', '--'
		...
		if(this.pointers['value2']) {
            ...
            if(isNaN(d.needle)) { d.needle=null }
            if (d.needle!=null) {              
                let val= (this.getAngle(d.needle) - this.ptrOffset);
                this.renderer.setAttribute (
		...
		if(this.pointers.hi) {
            ...
            if(isNaN(d.windtrue)) { d.windtrue=null }
            if(d.windtrue!=null) {
                let val= (d.windtrue<0) ? d.windtrue : this.getAngle(d.windtrue) - this.ptrOffset;
                this.renderer.setAttribute (
		...
		if(this.pointers.lo) {           
            ...
            if(isNaN(d.windapparent)) { d.windapparent=null }
            if(d.windapparent!=null) {
                let val= d.windapparent;
                this.renderer.setAttribute (
