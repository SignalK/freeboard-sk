...
export class TrackHistoryService {
  ...
  constructor() {
    // a server without a provider (or a reconnect to one) shows nothing
    effect(() => {
      if (!this.available()) {
  ...
  refreshRecorded(force = false) {
    ...
    this.get(
      ...
      next: (r) => this.recorded.set(parseHistoryContexts(r)),
      error: () => {
        this.recordedAt = 0;
      }
    });
  ...
  private fetchTrack(context: string) {
    ...
    req.subscribe({
      next: (fc) => {
        ...
        const track = parseHistoryTrack(context, fc, provider);
        this.tracks.update((m) => {
          const n = new Map(m);
          if (track) {
            n.set(context, track);
