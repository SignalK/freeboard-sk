...
export class FileInputComponent {
    ...
    @Input() label: string= 'Upload File';    // ** button text
    ...
    readFiles(files, index=0) {
        ...
        if(index in files) {
            ...
            // ** Check file type **
            if( this.accept.indexOf('/*')!=-1 ) { // ** if <filetype>/* used in accept attribute
                let ftype= files[index].type.substring(0, files[index].type.indexOf('/') );
