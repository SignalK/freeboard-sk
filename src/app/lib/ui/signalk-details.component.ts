...
export class SignalKDetailsComponent  {
    ...
    ngOnChanges() { 
        if(this.details) { this.parseEntries() } 
        else { this.items= [] }
    }

    // ** parse items
    parseEntries() { 
        let u= Object.entries(this.details);
        u.sort( (a,b)=> { return (a[0]<b[0]) ? -1 : 1 } );
    ...
    private section(u:Array<any>):Array<any> {
        ...
        // ** sort **
        result.sort( (a,b)=> { return (a[1]<b[1]) ? -1 : 1 } );
    ...
    private processObject(parent:string, obj:any) {
        let paths= [];
        let u= Object.entries(obj);
        u.sort( (a,b)=> { return (a[0]<b[0]) ? -1 : 1 } );
        u.forEach( i=> {
            if(typeof i[1]==='object' && i[1]!=null) {
                paths= paths.concat( this.processObject(parent + '.' + i[0],i[1]) );
            }
            else { paths.push([ parent + '.' + i[0], i[1] ]) }
        });
        return paths;
    }
